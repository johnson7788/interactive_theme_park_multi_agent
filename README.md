...
1. 打卡点配置的事件类型，编辑打卡点，编辑打卡点的事件类型关联1个奖励， 1个打卡点有1个奖励。 --- 怎么被触发（用户打卡）
2. 机器人的prompt动态获取，从内存获取。 
3. 用户创建接口，被微信小程序调用, 咱们这自动创建。使用 xiaozhi_web/supabase/20251108_create_admin_system_schema.sql 借助fastapi 创建1个api接口，自动创建1个用户到supabase。
4. 创建奖励，奖励是总的奖励。
5. 当前游乐园，当有多个游戏时，游戏主题管理加上激活选项。
6. 前端小智说话时，麦克风关闭。