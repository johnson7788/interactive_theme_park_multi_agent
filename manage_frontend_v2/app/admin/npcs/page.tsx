...
import { api, type NPCCharacter, type GameTheme } from '@/lib/api';
...
export default function NPCsPage() {
  ...
  const fetchData = async () => {
    try {
      const [npcsData, themesData] = await Promise.all([
        api.npcs.getAll(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitData = {
        ...formData,
        game_theme_id: formData.game_theme_id ? parseInt(formData.game_theme_id.toString()) : 0
      };

      if (selectedNpc) {
        await api.npcs.update(selectedNpc.id, submitData);
        toast.success('NPC更新成功');
      } else {
        await api.npcs.create(submitData);
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setFormData({ ...formData, game_theme_id: parseInt(value) })
                  }
                >
                  <SelectTrigger>
